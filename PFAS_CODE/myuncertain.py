...
def start_lr_forecast_uncertain(df_sem_o, df_raw_o, best_params, selected_features, path_forecast_input, save_path_lr, forecast_type, list_pfas):
    df_sem = df_sem_o.copy()
    df_raw = df_raw_o.copy()

    remain_col = ['lon_grid', 'lat_grid', 'year_raw', 'value']
        ...
        for i in range(2000,2021):
            for pfas in list_pfas:
                ...
                for sp_index in [0,1,2,3,4]:
                    ...
                    df_reture['sp_value'] = df_reture['value'] * std + mean
                    df_reture['sp_value'] = inv_boxcox(df_reture['sp_value'], fitted_lambdas)
...
def start_sw_forecast_uncertain(df_sem_o, df_raw_o, best_params, selected_features, path_forecast_input, save_path_sw, forecast_type, list_pfas):
    df_sem = df_sem_o.copy()
    df_raw = df_raw_o.copy()
    ...
    for seed in random_seeds:
        ...
        for i in range(2000,2021):
            for pfas in list_pfas:
                safe_pfas = pfas.replace(':', '-').replace(' ', '-').replace('/', '-')
                df_forecast_data = pd.read_csv(path_forecast_input + 'sw_'+str(i)+'_' + safe_pfas + '.csv')
                df_forecast_data['year_raw'] = df_forecast_data['year']
                df_forecast_data['year'] = (df_forecast_data['year'] - 2000) / (2020 - 2000)
                ...
                df_reture['sw_value'] = df_reture['value'] * std + mean

                df_reture['sw_value'] = inv_boxcox(df_reture['sw_value'], fitted_lambdas)
...
def merge_year(path_input, path_output, list_list_pfas,file_prefix, str_y='value', max_year=2021):
...
def process_csv_files(input_path, output_path, columns, file_prefix):
    ...
    for col in columns:
        ...
        for csv_file in csv_files:
            ...
            col_names.append(new_col_name)  # 添加新列名到列表
...
def process_pfas_data(base_folder_path, output_base_path, list_pfas):
    """
    处理PFAS数据，计算平均值并保存结果。
